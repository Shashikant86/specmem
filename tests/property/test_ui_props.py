...
class TestFilterCorrectness:
    ...
    def test_combined_filters_use_and_logic(
        ...
        filtered = filter_blocks(
            blocks, status=status.value, block_type=block_type.value
        )
        for block in filtered:
...
class TestSearchResultOrdering:
    ...
    @given(st.lists(st.floats(min_value=0.0, max_value=1.0, allow_nan=False), min_size=2, max_size=20))
    @settings(max_examples=100)
...
class TestPortConfiguration:
    ...
    def test_find_available_port_returns_valid_port(self, preferred: int):
        ...
        from specmem.ui.server import find_available_port, is_port_available
        try:
