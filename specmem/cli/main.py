...
from specmem.vectordb import LanceDBStore, get_embedding_provider, list_backends, SUPPORTED_BACKENDS
...
def vector_backend(
    ...
    if backend != config.vectordb.backend:
        console.print(f"[yellow]Warning:[/yellow] Switching from {config.vectordb.backend} to {backend}")
...
def infer(
    ...
    for i, candidate in enumerate(candidates, 1):
        ...
        confidence_color = "green" if candidate.confidence >= 0.7 else "yellow" if candidate.confidence >= 0.5 else "red"
        
        console.print(f"[bold]{i}.[/bold] {candidate.title}")
        console.print(f"   Type: [{candidate.spec_type.value}]")
        console.print(f"   Confidence: [{confidence_color}]{candidate.confidence:.2f}[/{confidence_color}]")
...
def graph_impact(
    ...
    from specmem.impact import SpecImpactGraph, GraphBuilder
...
def graph_show(
    ...
    if outgoing:
        ...
        for edge in outgoing:
            ...
            console.print(f"  → {edge.target_id} ({edge.relationship.value}, {edge.confidence:.2f}){manual}")
    ...
    if incoming:
        ...
        for edge in incoming:
            manual = " [manual]" if edge.manual else ""
            console.print(f"  ← {edge.source_id} ({edge.relationship.value}, {edge.confidence:.2f}){manual}")
...
def graph_export(
    ...
    filter_type: str = typer.Option(None, "--filter", help="Filter by node type (spec, code, test)"),
    ...
    """
    from specmem.impact import SpecImpactGraph, NodeType
...
def spec_drift(
    ...
    for item in drifted[:20]:
        ...
        table.add_row(
            ...
            item.suggested_action[:40] + "..." if len(item.suggested_action) > 40 else item.suggested_action,
        )
...
@app.command("stale")
def spec_stale(
    acknowledge: str = typer.Option(None, "--acknowledge", "-a", help="Acknowledge staleness for spec:version"),
    path: str = typer.Option(".", "--path", "-p", help="Repository path"),
) -> None:
    ...
    if format == "json":
        ...
        if result.issues:
            ...
            for issue in result.issues[:20]:
                sev_color = "red" if issue.severity.value == "error" else "yellow" if issue.severity.value == "warning" else "blue"
                table.add_row(
